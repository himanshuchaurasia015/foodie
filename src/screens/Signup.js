...
export default function Signup() {
  ...
  const handleClick = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/auth/getlocation",{ latlong: { lat, long } }, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/auth/createuser",{ name: credentials.name, email: credentials.email, password: credentials.password, location: credentials.geolocation }, {
        headers: {
          'Content-Type': 'application/json'
        },
      }).then(async()=>{
